...
> 本文件是**目标层级的唯一真相**。不要从 `huhuhulululu/kalshi`（15 分钟盘）抄日均美元阶段。
> 目标由用户设定；Agent 不发明 $10/$50/$200 天目标。
...
找不到下单点是策略问题，不是「盘口没机会」。不能实现就换策略，不要靠放宽门硬成交。
...
- **失败信号**：静/弱阳乱挂、垫档、吃 taker、整条切 live、为成交改门
