...
兑现」。1544 小时 / 66.2 天的回测把兑现那一半推翻了：同一批 **348 张** coupon 成交，
**持到结算 −0.86¢（t=−0.33，公道），套上 10–50% 带 / −80% 止 / 8 分钟 scratch 之后
−4.14¢（t=−4.99）**，日历前后半都显著。赢家被截在 +10%、输家吃满止损，是不对称截断。
...
定义不能建在样板上：**十七张真单里挑出来的两张，方向相反**。改定义的依据是 348 张成交的
每张净值，不是 `AUG2608`。`AUG2520` 仍是金带子，且两种收尾都赢。
