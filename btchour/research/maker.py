...
from btchour.research.metrics import mean_ci, t_stat
    ...
    *,
    ...
    for strike, sticks in tape.candles.items():
        ...
        for index, end_ts in enumerate(stamps[:-1]):
            ...
            if bid is not None and traded_low is not None and traded_low <= bid:
                fills.append(
                    MakerFill(
            ...
            if ask is not None and traded_high is not None and traded_high >= ask:
                fills.append(
                    MakerFill(
...
def scan_tapes(tapes: list[EventTape], **kwargs) -> dict:
    ...
    for tape in tapes:
        if tape.error:
            continue
        hours += 1
        fills.extend(scan_tape(tape, **kwargs))
    report: dict = {"hours": hours, "fills": len(fills), "sides": {}}
...
def render(report: dict) -> str:
    lines = [
        ...
        "",
        "不设门、不设出场：挂在该档自己的买一/卖一，下一分钟真有打穿才算成交，",
        "maker 费 0。忽略排队，所以这是上界。",
        "",
