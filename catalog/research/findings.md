...
卖出收到 `Σbid`，最多赔 1，`Σbid > 1` 就是锁死的利润。**6 轮快照、每轮约 13450 个
事件，共 23765 次观测，0 次违反。** 最接近的一次是 **−0.01¢**（差一分钱的百分之一
就交叉了，但没交叉），中位数 **−11.6¢** —— 典型互斥事件的抽水有 11 分宽。
...
**嵌套阶梯。** **8 轮快照，每轮覆盖约 12480 个在挂市场**，共找到 **54 次交叉**，
集中在**两个**事件里（都是 NCAAF 上半场让分），**54 次全部是 0.01 张** ——
最大的一次机会值 **$0.0003**。每轮 6–8 次，稳定得像个常数，而那个常数是零。
...
## 常规量化的结构账（决策 021，无新数据，纸面）

`btchour/research/costfloor.py` 复现：

```bash
python3 -c "
from btchour.research.costfloor import *
for v in VENUES: print(v.name, quoting_ratio(v), round_trip_cost_bp(v, MAKER_BOTH))
"
```

**费率与价差**（`verified=True` 的行有出处，其余待联网核实）

| 标的 | 价差 | taker | maker | 挂单比值 `价差/2×maker` | 两边挂满一来回 |
| --- | --- | --- | --- | --- | --- |
| 币安 USDT-M `BTCUSDT` 永续 VIP0 | 0.09bp | 5.0bp | 2.0bp | **0.022** | **−3.91bp** |
| 币安现货 `BTCUSDT` VIP0 | 0.09bp | 10.0bp | 10.0bp | **0.004** | **−19.91bp** |
| 美股大盘（未核实） | 0.5bp | 0.28bp | 0.28bp | 0.89 | −0.06bp |
| CME ES（未核实） | 0.5bp | 0.10bp | 0.10bp | 2.50 | +0.30bp |
| Kalshi `KXBTCD`（已实测） | 107bp | 175bp | **0** | **∞** | +107bp |

读法：Kalshi 这一行结构上**好两个数量级**（不收挂单费），020 仍然测出挂单方的钱
不存在。加密那两行在对手方完全不知情时就已经每来回亏 4–20bp。**逆向选择根本没机会
上场。**

**打平比例**（吃单来回成本 / 一倍 sigma，币安永续 VIP0，BTC 年化波动按 50%）

| 持仓期 | sigma | 吃单来回 | 挂进吃出 |
| --- | --- | --- | --- |
| 1 分钟 | 6.9bp | 146.3% | 101.5% |
| 1 小时 | 53.4bp | 18.9% | 13.1% |
| 1 天 | 261.7bp | 3.9% | 2.7% |
| 30 天 | 1433.5bp | 0.7% | 0.5% |
| 美股单只 1 天（未核实） | 189.0bp | 0.56% | — |
| 美股单只 20 天（未核实） | 845.2bp | 0.13% | — |

**现货-永续 carry**（两腿来回 24bp：现货 10bp × 2 + 永续挂单 2bp × 2）

| 资金费率口径 | 8 天 | 30 天 | 90 天 |
| --- | --- | --- | --- |
| 长期基准 0.01%/8h ≈ 11%/年 | 净 0.00%/年 | 净 8.03% | 净 9.98% |
| 2026-09-09 读数 0.0078%/8h | 净 −2.41%/年 | 净 5.62% | 净 7.57% |
| 扣 1 年期美债 4.45% 后的超额 | −4.5 ~ −6.9pp | **+1.2 ~ +3.6pp** | **+3.1 ~ +5.5pp** |

单次读数不是均值，完整历史要等白名单。但成本结构已经确定了两件事：
**持仓短于约 8 天必然为负**；**超额是个位数百分点，换来的是交易所破产 / 强平 /
托管风险**。旁证：公开仓库 `aoki-h-jp/funding-rate-arbitrage` 的示例输出扣完
0.32% 手续费是 **每 100 USDT −0.1406**。

## H3 波动率风险溢价（Deribit），决策 024，2026-09-20

数据：Deribit `get_volatility_index_data`（DVOL 日线 OHLC，BTC/ETH 各 2007 天，
2021-03-24 → 2026-09-20）、`get_delivery_prices`（真实交割指数，btc_usd 2614 天
回溯到 2016-03）、`get_tradingview_chart_data`（月度期权日线，去掉到期后与
零成交量的幽灵 K 线）、币安小时现货（`data.binance.vision`，58409 小时）。

### 第一层：DVOL vs 实现波动率，不重叠 30 天窗口，实现值用小时线

| | n | 隐含均值 | 实现均值 | 方差溢价（点） | 区间 | t | 最差 | top5 占比 |
|---|---|---|---|---|---|---|---|---|
| BTC 全样本 | 66 | 61.74 | 52.59 | +6.95 | [+3.39, +10.35] | 3.88 | −34.51 | 37% |
| BTC 前半 | 33 | 73.30 | 58.84 | +11.27 | [+5.63, +16.74] | 3.90 | −34.51 | 45% |
| BTC 后半 | 33 | 50.18 | 46.34 | +2.63 | [−1.37, +6.08] | 1.40 | −32.68 | 83% |
| BTC 近 12m | 12 | 42.91 | 42.37 | −1.14 | [−9.32, +5.65] | −0.29 | −32.68 | 100% |
| ETH 全样本 | 66 | 75.67 | 69.43 | +3.17 | [−2.86, +8.75] | 1.07 | −116.81 | 94% |
| ETH 后半 | 33 | 65.14 | 64.52 | −1.05 | [−6.55, +4.05] | −0.38 | −45.53 | 100% |
| ETH 近 24m | 24 | 65.01 | 65.17 | −1.98 | [−8.81, +4.36] | −0.58 | −45.53 | 100% |

日线实现值给的结论一样（BTC 全样本 +6.75、t=3.05），小时线只是把区间收窄、
尾部变小，所以估计量不是问题。

功效补测：
- 起点相位扫描（30 个不重叠切分，每个错开一天）。BTC 全样本 **30/30 个相位
  t>2**（t 范围 +2.64~+4.23，均值 +6.11）；近 24 个月 **4/30**（+1.72~+4.75）；
  近 12 个月 **0/30**（−2.12~+4.21）。
- 全起点（日度滚动）+ 分块自举（块长 = 30 天）：BTC 全样本 1957 个起点
  [+3.42, +8.77]；近 24 个月 701 个起点 [−0.84, +5.22]；近 12 个月 336 个起点
  [−5.40, +4.27]。ETH 全样本 [−1.83, +7.01]。
  **同一组数据按 iid 自举，区间窄 4.3 倍**（近 24 个月会变成 [+1.86, +3.28]，
  假显著）。

### 口径拆解：DVOL 不是平价隐含

从跨式价格反解平价隐含波动率（逆向结算，币本位跨式价
`2·(2Φ(σ√T/2)−1)`，二分反解）。BTC 62 个到期：

| | 波动率点 | 区间 | t |
|---|---|---|---|
| DVOL − 实现 | +7.22 | [+3.23, +11.00] | 3.64 |
| 微笑凸性 DVOL − 平价隐含 | **+3.75** | [+2.95, +4.58] | **8.82** |
| 平价卖方拿到的 平价隐含 − 实现 | +3.46 | [−0.41, +7.11] | 1.81 |
| 同上，近 24 个月 | −0.95 | [−5.31, +2.90] | −0.45 |

均值：DVOL 58.67 / 平价隐含 54.92 / 实现 51.45。ETH 楔子 +4.90（t=9.13），
平价卖方全样本 −1.27、近 24 个月 −5.27。

### 第二层：卖月度平价跨式持有到结算

|价外程度| 中位数 BTC 1.40%（最大 2.99%）、ETH 1.15%（最大 2.86%），
持有期中位数 29 天。权利金和 payoff 都是名义额百分比。

| | n | 权利金 | payoff | 毛损益 | 区间 | t | 胜率 | 最差 |
|---|---|---|---|---|---|---|---|---|
| BTC 全样本 | 62 | 12.33% | 12.89% | −0.56% | [−3.26, +1.97] | −0.42 | 61% | −31.73% |
| BTC 后半 | 31 | 10.53% | 10.77% | −0.24% | [−3.52, +2.74] | −0.15 | 68% | −23.54% |
| BTC 近 12m | 13 | 9.13% | 11.26% | −2.13% | [−8.07, +3.05] | −0.72 | 69% | −23.54% |
| ETH 全样本 | 61 | 14.88% | 17.45% | −2.57% | [−6.44, +1.17] | −1.31 | 52% | −48.60% |
| ETH 近 24m | 25 | 13.81% | 18.42% | −4.61% | [−10.43, +0.60] | −1.72 | 48% | −37.91% |

第一版用固定美元行权价步长，ETH 的 |价外| 中位数是 5.88%、最大 45.74%，
毛损益 −3.97%——那是在测方向不是测波动率。改成比例网格后才是上表。

### 执行成本（这次不是凶手）

Deribit 实盘盘口，2026-09-20，40 天到期平价：

| | 买价 | 卖价 | 价差/中值 | 顶档量 |
|---|---|---|---|---|
| BTC-30OCT26-81000-C | 0.0440 | 0.0450 | 2.25% | 12.7 BTC |
| BTC-30OCT26-81000-P | 0.0450 | 0.0460 | 2.20% | 1.6 BTC |
| ETH-30OCT26-2600-C | 0.0615 | 0.0630 | 2.41% | 38 ETH |
| ETH-30OCT26-2600-P | 0.0655 | 0.0670 | 2.26% | 38 ETH |

平价附近 12 个行权价价差中位数 BTC 2.56%、ETH 2.26% 的中值。
跨式穿价 BTC **0.10 个百分点名义额**（权利金的 1.11%）、ETH 0.15 个百分点。
Deribit 费用：开两腿 0.06% + 实值腿交割 0.015% = 0.075 个百分点。
**全部执行成本每个到期 0.21 个百分点，毛损益 −0.56 个百分点。**

| BTC 跨式净额 | 每到期 | 年化 | 区间 | vs 美债 4.45% |
|---|---|---|---|---|
| 全样本 62 个 | −0.78% | −9.78% | [−43.72, +22.14] | −14.23pp |
| 近 24 个月 25 个 | −0.78% | −9.87% | [−58.36, +33.18] | −14.32pp |

### 顺带更正 021 和上一次汇报

021 把 H3 排第三的理由之一、以及上一次汇报里我写的「Deribit 价差宽」，
都是错的：**Deribit 平价期权盘口是窄的**（跨式穿价 0.1 个百分点、
顶档约 $100 万名义）。021 的另一个理由——收割路径方差需要对冲模型——仍然成立，
而且是 H3 剩下唯一没被测的形状。

## 025 换机制：四类新机制（2026-09-20）

数据全部来自 `data.binance.vision` 归档（不分页、不可变、带 SHA-256）。

### M1 币安 USDT-M 永续挂单池

面板：20 个币跨五个 ADV 分位 × 12 天（2025-09 ~ 2026-08 每月 10 日），
**232 个币日、$3888 亿名义额**。方法：`aggTrades` 的 `is_buyer_maker` 给出被动方，
`markout = side * (ref_{t+h} − fill) / fill`，参考价用 t+h 附近的 VWAP 窗口
（末笔成交版互校：BTC −0.58 vs −0.61，DOGE −0.90 vs −0.96）。
独立单位 = 币日，按名义额加权。

| 60s markout | 毛（vw） | 毛（等权） | 费 | 净（vw） | 区间 | t | 正天数 |
|---|---|---|---|---|---|---|---|
| retail | −0.48 | −0.78 | +2.0 | −2.48 | [−3.40,−2.07] | −8.23 | 4% |
| vip1 | −0.48 | −0.78 | +1.6 | −2.08 | [−3.00,−1.67] | −7.05 | 5% |
| vip3 | −0.48 | −0.78 | +1.4 | −1.88 | [−2.80,−1.47] | −6.46 | 7% |
| vip5 | −0.48 | −0.78 | +1.0 | −1.48 | [−2.40,−1.07] | −5.27 | 9% |
| vip7 | −0.48 | −0.78 | +0.6 | −1.08 | [−2.00,−0.67] | −4.08 | 11% |
| **vip9** | −0.48 | −0.78 | **−0.5** | **+0.02** | **[−0.90,+0.43]** | −0.82 | 34% |

持有期扫描（retail）：1s −2.38、10s −2.43、60s −2.48、300s −3.86bp
（逆向选择随持有期继续增长，最差单日 −125.56bp）。
vip9 两个不重叠半段：+0.12（t=0.09）/ −0.19（t=−2.40）。

ADV 分位毛额（费前，vw）：q1 −0.47、q2 −2.56、q3 −0.27、q4 −1.82、q5 **+3.63**。
q5 等权只有 −0.54——正号来自少数高成交量日。

单币毛额（费前，vw，12 天）：
AWE +10.25、MASK +5.46、JST +5.46、KNC +4.26、UMA +2.40、ID +0.69、
BTC −0.07、ETH −0.33、ZEC −0.54、ROBO −0.57、DOGE −0.96、PEOPLE −1.19、
AT −1.21、KAITO −1.84、BIO −1.66、NOM −2.47、SOL −2.74、ACT −3.29、
DOGS −3.65、GTC −3.73、SAND −7.34。
5 个超过 2bp 零售挂单费的币，**每个等权远低于加权、区间全跨 0**
（t：JST +0.67、MASK +0.44、UMA −1.07、KNC +0.11、AWE +0.44），
4 个的单日贡献 >100% 总额，合计名义额占面板 **0.07%**。
毛额对 log10(ADV) 斜率 **−0.59bp/十倍**、相关 −0.19（n=19）——「越薄越好」不成立。

### M2 现货三角静态套利

4 个三角 × 5 天 = 20 个币日，**251,006 次三腿同时真成交观测**。
窗口 1000ms，陈价丢弃 7–33%（硬闸设在 50%）。
|edge| 中位数 1.19–2.49bp，p99 3.5–10.3bp。

| 档 | 成本地板 | 违反次数 | 总额 | 折年 |
|---|---|---|---|---|
| retail | 30.0bp | 84 | $0.017 | $1.23 |
| bnb | 22.5bp | 108 | $0.025 | $1.83 |
| vip9 | 5.1bp | 2846 | $0.067 | **$4.91** |

存活者三腿陈价中位数 **0ms**，窗口 1000→200ms 存活者 701→696（不是陈价造成的）。
最薄腿名义额：p50 **$0.00**、p90 $0.13–0.22、最大 **$1.10**。

### M4 单边流爆发的对手方

12 个币 × 24 个月，每币 1,051,200 分钟。条件：|imbalance| ≥ X、|move| ≥ Y、
成交额 ≥ 3× 前 60 分钟中位数、move 与 imbalance 同向。入场 = 下一根开盘。
成本地板 13.99bp（3.99 价差 + 2×5 taker）。

| imb / move | n | 币日 | 毛回归 | 区间 | 净 | t | 正比例 |
|---|---|---|---|---|---|---|---|
| 0.5 / 10bp | 161094 | 8733 | +0.44 | [+0.25,+0.62] | −13.55 | −52.85 | 30% |
| 0.5 / 20bp | 56114 | 7983 | +0.93 | [+0.53,+1.36] | −13.06 | −38.80 | 36% |
| 0.5 / 50bp | 4280 | 2484 | **+5.71** | [+2.93,+8.61] | −8.28 | −5.23 | 46% |
| 0.6 / 10bp | 101904 | 8642 | +0.29 | [+0.09,+0.50] | −13.70 | −89.91 | 28% |
| 0.6 / 20bp | 30963 | 7075 | +0.76 | [+0.30,+1.24] | −13.23 | −44.15 | 35% |
| 0.6 / 50bp | 1641 | 1230 | **−1.00** | [−4.99,+2.75] | −14.99 | −6.68 | 44% |
| 0.8 / 10bp | 26789 | 6116 | +0.16 | [−0.16,+0.48] | −13.83 | −52.43 | 25% |
| 0.8 / 20bp | 5751 | 2586 | +1.40 | [+0.50,+2.30] | −12.59 | −20.56 | 32% |
| 0.8 / 50bp | 106 | 104 | +0.03 | [−14.52,+13.42] | −13.96 | −1.97 | 43% |

持有期（0.6/20bp）：1min +0.60、5min +0.39、10min +0.76、30min +0.86bp。
最差 −1187.9bp，top5 占比处处 100%。

### M3 资金费结算时点

**理论被证伪**：条件在大额滞后资金费上，`signed_into_bp` = −16.94bp
（≥5bp 档），即价格**顺着**资金费符号动进结算点，不是逆着。没有离场人群。

majors（12 个币、24 个月、26,268 个结算、8,760 个币日、8 小时结算）：
平均 |rate| 仅 0.77bp，交易 −11.04bp [−12.04,−10.03] t=−21.27。
窗口扫描 ±5/15/30/60 分钟：−10.65 / −10.94 / −11.04 / −9.94bp。
**majors 上没有东西可躲**（|资金费| 0.77bp vs 成本 13.99bp）。

按 ADV 分层面板（20 个币，2025-09~2026-08）、**严格滞后信号**、按币日自举：

| |滞后 rate| | n | 币日 | 币 | 交易 | 区间 | t | 正比例 | top5 |
|---|---|---|---|---|---|---|---|---|
| ≥0bp | 36958 | 8128 | 20 | −10.75 | [−11.96,−9.50] | −18.67 | 42% | 100% |
| ≥2bp | 4549 | 1533 | 20 | +11.43 | [+6.47,+16.43] | +0.33 | 51% | 14% |
| **≥5bp** | 2156 | 728 | 19 | **+28.99** | [+20.71,+37.15] | **+2.80** | 57% | 11% |
| ≥10bp | 1123 | 377 | 17 | +56.85 | [+43.75,+70.44] | +6.71 | 65% | 10% |
| ≥20bp | 504 | 192 | 17 | +88.62 | [+64.67,+115.01] | +5.44 | 71% | 14% |
| 前半 ≥5bp | 1078 | 364 | 18 | +23.62 | [+13.24,+34.76] | +2.18 | 56% | 24% |
| 后半 ≥5bp | 1078 | 364 | 16 | +34.36 | [+21.33,+46.95] | +1.76 | 58% | 15% |

用已实现（前视）信号同一面板：≥5bp 给 +33.60 t=+5.99——**前视值约 5bp 和 3 个 t**。
按「平均资金费最高」选样的版本（有选样偏差，仅供对照）：≥5bp +15.14 t=+5.14。

分解（≥5bp）：+28.99 = 价格 +28.82 + 收到资金费 18.77 − 成本 13.99。

单币（≥5bp，n≥20）：NOM +58.91 t=3.72、AWE +56.13 t=3.17、KAITO +45.41 t=2.96、
AT +37.19 t=1.27、UMA +25.04 t=0.26、BIO +20.28 t=0.90、KNC +19.70 t=−0.12、
ROBO +13.06 t=−0.48、GTC +4.77 t=−0.18、ID +4.56 t=−0.43、SAND +0.52 t=0.10、
**JST −16.93 t=−2.06**、ACT −17.98 t=−1.39、DOGS −25.10 t=−1.34。
15 个里 3 个单独显著为正，1 个显著为负。

**实测成本**（`spread.py` 成交指示回归，承载结果的 8 个币 × 3 天）：
NOM 4.27、KAITO 2.75、AWE 3.65、BIO 3.75、ID 3.72、UMA 3.27、AT 3.45、JST 2.70bp，
中位数 **3.55bp** → 往返 7.10bp 价差 + 10bp taker = **17.10bp**（原假设 13.99bp）。

成本敏感度（≥5bp）：往返价差 4.0bp → +28.99 t=+2.80；8.0bp → +24.98 t=+1.96；
15bp → +17.98 t=+0.50；25bp → +7.98 t=−1.60；40bp → −7.02 t=−4.74。
**打平点在往返价差 25bp 附近，实测 7.10bp，余量三倍多。**

容量：事件 ±30 分钟窗口成交额 p10 $127k、**p50 $1.33M**、p90 $13.7M。
1% 参与度每次中位 $13,342，19 个币合计毛 **$696k/年**；5% 参与度 **$3.48M/年**。
