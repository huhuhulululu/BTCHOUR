...
## 公开股票异象的发表后衰减（决策 022，Ken French 库，1926-07 ~ 2026-07）

复现：

```bash
python3 -m btchour research factors            # 表格
python3 -m btchour research factors --json     # 机器读
```

数据是 `data/archive/frenchlib/` 里的 8 个 zip，整包下载，**不分页**。
区间一律按**日历年整块重抽**（`cluster_ci`，groups = 年），不按月。

**发表后衰减**（每个异象按自己的原始样本终点 / 发表日切）

| 因子 | 原始论文 | 样本内 | 样本外未发表 | 发表后 | 发表后 95% 区间 | 存活 |
| --- | --- | --- | --- | --- | --- | --- |
| 市场（对照） | — | 7.67%/y t=3.09 | 16.04%/y t=1.83 | 9.14%/y t=3.48 | (3.30, 14.55) | **是** |
| 规模 SMB | Banz 1981 | 2.30%/y t=1.40 | 14.68%/y t=3.62 | 0.18%/y t=0.12 | (−2.29, 2.61) | 否 |
| 价值 HML | FF 1992 | 5.27%/y t=3.30 | 8.19%/y t=1.04 | 2.17%/y t=1.12 | (−2.64, 7.02) | 否 |
| 动量 Mom | JT 1993 | 8.53%/y t=4.15 | 14.01%/y t=2.24 | 4.67%/y t=1.63 | (−1.46, 10.20) | 否 |
| 盈利 RMW | Novy-Marx 2013 | 3.31%/y t=2.88 | 1.57%/y t=0.43 | 2.55%/y t=1.12 | (−1.28, 6.81) | 否 |
| 投资 CMA | TWX 2004 | 3.66%/y t=3.26 | 9.04%/y t=2.32 | −0.06%/y t=−0.04 | (−3.59, 3.96) | 否 |
| 投资（备选断点） | CGS 2008 | 4.61%/y t=3.98 | −2.19%/y t=−1.12 | 0.56%/y t=0.33 | (−3.64, 5.38) | 否 |

**对照组是这张表的关键。** 市场溢价发表后**更强**（t=3.48，区间不跨 0），
所以切分点抓到的不是某个宏观状态。而「样本外但尚未发表」那一段大多仍然强，
说明死亡发生在**发表**，不在样本结束。

**发表后按规模五分位拆开**（市值份额取 202607，全市场合计 $68.85 万亿）
...
| 动量 PRIOR5−PRIOR1 | 年化 | t | 95% 区间 | 市值份额 |
| --- | --- | --- | --- | --- |
| 最小 | 9.07% | 2.71 | (−0.19, 17.31) | 0.67% |
| 小 | 6.82% | 1.92 | (−2.19, 14.67) | 1.55% |
| 中 | 5.39% | 1.43 | (−2.32, 12.55) | 2.89% |
| 大 | 6.20% | 1.48 | (−1.42, 13.47) | 7.60% |
| 最大 | 4.61% | 1.10 | (−3.62, 13.05) | 87.29% |

**动量五个桶一个都不跨零。** 全表只剩小盘价值一个候选，而它过不了不重叠窗口：

| 小盘价值，发表后 | n | 年化 | t | 95% 区间 | 最好 5% 占比 |
| --- | --- | --- | --- | --- | --- |
| 1992-07 ~ 2009-12 | 210 | 12.99% | 3.07 | (3.63, 21.99) | 54.3% |
| 2010-01 ~ 2026-07 | 199 | 6.46% | 1.58 | (−3.61, 18.46) | **115.9%** |

近窗占比超过 100% 的意思是**去掉最好的十个月，剩下的合计为负**。全窗最好的十二个月：

```
202101 +24.70%   202103 +17.31%   200011 +16.09%   200204 +14.80%
202201 +13.96%   200004 +13.07%   200102 +12.82%   200012 +12.34%
200201 +12.08%   202204 +12.06%   201909 +11.51%   200003 +11.22%
```

七个落在 2000-04 ~ 2002-04，两个落在 2021-01 ~ 2021-03。两次事件，不是一门生意。

**成本地板**（打平来回成本 = 年化溢价 / 2×年换手；多空各一腿）
...
**这一次成本不是凶手。** 价值按年重排，换手约 1 次/年，能扛 490bp 来回，
微盘价差吃不掉。杀死它的是区间跨零和收成形状。做空微盘的借券成本是唯一没量到的
输入（没有股票报价源），**所以它没有进结论**。
