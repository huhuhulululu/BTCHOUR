...
## C 方向的预判 + 这个市场的结构账（2026-09-18，用户「你来优选」）

### 8. 波动率地板已改，模型现在和中价打平

`annual_vol` 一个数同时干了两件事：给**测得的**波动率当地板，和在测不出来时当**兜底**。
0.55 当兜底是保守（测不出来时宁可高估），当地板是灾难（把每个 p 往 0.5 拽）。已拆开：

- `Settings.vol_floor = 0.20` —— 地板。`realized_annual_vol` 内部就 clamp 在 0.25，
  所以这个地板**实际不起作用**，模型用它测到的值。
- `Settings.annual_vol = 0.55` —— 只在样本不足、测不出 realized 时兜底。

改完重跑（同样 92 小时）：

```
全样本 Brier：模型 0.10396   中价 0.10319      （改之前：模型 0.11519）

分歧               读数     模型 Brier     中价 Brier      谁更准
0-2¢          21649      0.07804      0.07816       模型
2-5¢          16939      0.10956      0.10973       模型
5-10¢          8099      0.14566      0.14351       中价
>10¢           1881      0.17237      0.15886       中价
```

**017 的结论没有变，而且更干净了。** 读数分布整个翻过来：分歧 ≥5¢ 的从 75.3% 掉到
20.5%，原来那些「分歧」大部分是地板造出来的假象。而**剩下的真分歧里，中价照样赢**。
也就是说：模型少犯错了，但它仍然没有市场不知道的信息。

回放里各 playbook 的变化（92 小时）：

| playbook | 0.55（旧） | 0.20（新） |
| --- | --- | --- |
| `flex`（默认） | 1 笔 / +0.84 | 1 笔 / +0.45 |
| `swing` | 37 笔 / −20.14 | **55 笔 / −19.14** |
| `lock` | 2 笔 / +9.77 | 3 笔 / +13.80 |

注意 `swing`：波动率降低让 p 变锐利，`p ≥ 52%` 这类门**更容易过**，所以它开火**更多**
（37 → 55 笔），而且照样是亏的。**校准变好不等于策略变好。** 默认的 `flex` 基本不受影响。
`swing` 在两种设置下、样本外两段都亏，**不要用**。

### 9. C（贴价挂，赚价差）的预判：价差不够厚

不用等 300 小时。决定 C 的那个数 —— **价差** —— 现在就能精确测，而它太薄了。

92 小时、40036 个 (分钟, 档位) 观察，挂 YES 买价、持到结算：

| | 笔数 | 买价边际 | 95% 区间（按小时聚类） |
| --- | ---: | ---: | --- |
| A 假设永远成交（**上限**，无逆向选择） | 40036 | **+0.0157** | [−0.0220, +0.0533] |
| B 只算下一分钟真跌到我价位的 | 23687 | **−0.0098** | [−0.0452, +0.0262] |

- **被成交的代价（逆向选择）= A − B = 2.55 分**
- **能赚的价差（平均 ask − bid）= 1.06 分**

**逆向选择是整条价差的 2.4 倍。** 挂单被打到，是因为对面知道得比你多；这个市场里这件事
的代价比你挂单能赚的全部还多一倍以上。

按价差宽度、按剩余时间切开，没有一个角落是正的：

| 只看真被打到的 | 笔数 | 买价边际 |
| --- | ---: | ---: |
| 价差 0.5–1.5 分 | 22563 | −0.0102 |
| 价差 1.5–2.5 分 | 1102 | −0.0022 |
| 剩 5–15 分钟 | 2167 | −0.0185 |
| 剩 15–30 分钟 | 5672 | −0.0106 |
| 剩 30–45 分钟 | 7408 | −0.0222 |
| 剩 45–60 分钟 | 8429 | +0.0039 |

价差 ≥2.5 分的只有 22 个观察 —— **没有「宽价差角落」可躲**，99% 的盘口就是 0.5–1.5 分。

### 10. 结构账：这张桌子的费和价差对不上

实时盘口（2026-09-18 14:20 UTC 左右）：

| series | 两边报价档数 | 平均价差 | ATM 价差 | 价差 / ATM taker 费 |
| --- | ---: | ---: | ---: | ---: |
| `KXBTCD`（本仓库做的） | 51 | 1.18 分 | 1.20 分 | **0.69** |
| `KXBTC`（区间盘） | 31 | 3.06 分 | — | 1.75 |
| `KXBTC15M` | 1 | 1.00 分 | — | 0.57 |

Kalshi taker 费是 `0.07 · p · (1−p)`，ATM 是 **1.75 分**（单边）。

于是两条路各自的账：

- **吃单**：1.75 分费 + 约 0.6 分半价差 ≈ **2.35 分**成本。要赚钱得稳定有 2.35 个百分点
  以上的真优势。而第 1 节测出来模型比市场**差** 1.2 个 Brier 点。
- **挂单**：费 0，毛赚 **1.06 分**价差，逆向选择 **2.55 分**。

**两条路是同一个死因：价差（1.2 分）比交易它的成本（吃单 1.75 分费 / 挂单 2.55 分逆向
选择）都薄。** 这不是门没调好，是这张桌子在这个尺度上没有给外部参与者留位置。

`KXBTCD` 的 **价差/费 = 0.69** 是一句话版本：**吃一次单的手续费，比整条买卖价差还贵。**

### 11. 尾部是不对称的：两尾不能并成一个桶

（另一个线程在追尾部挂单这条候选，这里留下我这边的数据以免两边重复。）

挂 YES 买价、持到结算，92 小时 / 44883 个观察：

| | 笔数 | 每张边际 | 95% 区间（小时聚类） |
| --- | ---: | ---: | --- |
| 低尾 mid<0.15 上限 | 11071 | −0.39 分 | [−2.66, +2.32] |
| 低尾 mid<0.15 真被打到 | 8171 | **−1.47 分** | [−3.31, +0.74] |
| 高尾 mid>0.85 上限 | 13990 | +2.58 分 | [+0.26, +4.50] |
| 高尾 mid>0.85 真被打到 | 8743 | **+1.77 分** | [−0.61, +3.71] |
| 两尾合计 真被打到 | 16914 | +0.20 分 | [−1.39, +1.78] |

（「上限」= 假设永远成交，无逆向选择无排队。）

**低尾亏、高尾赚，合并成 +0.20 分同时低估高尾、又藏住低尾在亏。** 这和第 2 节的校准表
自洽：市场在高尾**偏保守**（中价 0.958 → 真实 0.979），在低尾**偏贵**（中价 0.041 →
真实 0.033）。买便宜彩票亏、买近确定的赢，是一个自洽的故事。

两个限制，别跳过：

- 高尾 +1.77 分的区间**仍然跨 0**，92 小时判不动。
- 高尾档位薄，**排队位置在这里是决定性的**。边际本来只有 1–2 分，排在后面就没了。
  上表和任何「忽略排队」的数字一样，都是**上界**。
- 成交代理是「下一分钟最优买价 ≤ 我的价位」，粗糙；且是单边挂 + 持到结算，不是真做市的
  双边 round trip。

## 12. `KXBTC` 区间盘：宽价差是真的，但没有可赚的边际（2026-09-18）

018 把 `KXBTC` 列为「值得先花一天量一下」的候选，理由是实时价差 3.06 分，是 `KXBTCD`
的 2.6 倍。用户说「测完」，这是结果。

**合约结构不同**：`KXBTC` 是 $100 分桶（`B<价>`，蝶式），不是上下轨。一个桶只在 BTC 正好
落在那 $100 区间才付 1，所以单价低（平均中价 **0.121**），档位多（每小时 238 个）。
仓库原来的 tape 抓取只认 `KXBTCD` 的 ticker 规则，所以另写了一份（`data/kxbtc/`，32 小时）。

### 先说一个把结果翻过来的计量坑

我原来的成交代理是「下一分钟最优买价 ≤ 我的挂价」。**这个代理在流动性差的盘口会凭空
造出成交** —— 报价可以在**没有任何成交**的情况下漂下来。而这些幻影成交恰恰是**良性**的
（没有知情对手方），所以它会把逆向选择**压低**。

| 口径 | `KXBTCD` | `KXBTC` |
| --- | ---: | ---: |
| 代理判成交的 bar 里，**真有成交量**的比例 | **91.0%** | **35.1%** |

`KXBTCD` 流动好，代理没问题（所以第 9 节的结论不受影响）。`KXBTC` 有 **65% 是幻影成交**。
要求成交那根 bar 真有成交量之后：

| | 上限（永远成交） | 报价成交（旧口径） | **真成交（诚实）** |
| --- | ---: | ---: | ---: |
| `KXBTC` 每张边际 | +1.16 分 | +0.46 分 | **−0.03 分** [−1.61, +1.68] |
| `KXBTCD` 每张边际 | −0.58 分 | −2.74 分 | **−3.02 分** [−7.93, +2.07] |

**`KXBTC` 那个 +0.46 分是代理造出来的，诚实口径是 0。**

### 结构上 `KXBTC` 确实比 `KXBTCD` 好，但好到 0 为止

| | `KXBTCD` 上下轨 | `KXBTC` 区间盘 |
| --- | ---: | ---: |
| 价差 | 1.07 分 | **2.58 分** |
| 逆向选择（要求真成交） | 2.44 分 | **1.18 分** |
| 价差 − 逆向选择 | **−1.37 分** | **+1.40 分** |
| 平均中价 | ~0.5 | 0.121 |
| 平均 taker 费 | 1.75 分(ATM) | 0.67 分 |
| 价差 / 费 | 0.69 | **3.83** |
| **每张边际（真成交）** | **−3.02 分** | **−0.03 分** |

018 的猜测方向是对的：**`KXBTC` 的价差确实覆盖得住逆向选择**（2.58 > 1.18），而
`KXBTCD` 覆盖不住（1.07 < 2.44）。这是两张桌子的真实结构差别，不是噪声。

但**实测边际是 0**，不是正的。价差比逆向选择宽的那 1.40 分，被别的东西吃掉了（挂单要排队、
要在两边都报价、出场也有成本）。

按中价分档（真成交口径下样本变薄，只作参考）：近 ATM 的桶（中价 >0.30）是**明显负的**，
正的点估计都在便宜桶，且区间全部跨 0。
...
- 成交 **每小时 27,320 张**，名义 **每小时 $7,029**
- 同口径 `KXBTCD` 是 **每小时 1,798,929 张** —— **`KXBTC` 只有它的 1.5%**

宽价差和低成交量是同一件事的两面：没人报得紧，因为没人交易。

假设边际真是正的（实测是 0），容量上限：

| 吃下成交量 | 边际 0.5 分 | 边际 1.0 分 |
| --- | ---: | ---: |
| 5% | $6.83/小时 | $13.66/小时 |
| 10% | $13.66/小时 | $27.32/小时 |
| 25%（不现实） | $34.15/小时 | $68.30/小时 |

**最乐观的一格（吃下全市场四分之一、边际 1 分，两个都不成立）是每小时 $68。**

### 结论

`KXBTC` 不是那个宽价差暗示的机会。它比 `KXBTCD` **敌意小得多**（0 对 −3 分），结构比也
确实是正的，但：实测边际为 0、区间跨 0、成交量只有 1.5%、容量上限是每小时几十美元。

**不建议投入。** 018 的第 1 条候选到此关闭。第 2、3 条（跨场馆价差、做市返佣身份）仍未测。

口径说明：本节 `KXBTCD` 的数字取自 `data/archive/` 全部档位（未按 |strike−spot| ≤ 600
过滤），所以和第 9 节的 −0.98 分不同口径；第 9 节是近 ATM 过滤后的。两个口径下 `KXBTCD`
都是负的，结论不变。
