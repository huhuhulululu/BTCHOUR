...
默认不再只锁仓、也不只逆势刮头。`flex` 把几套玩法叠在一起，**先拿已经决定的 20%，再顺着 3 分钟动量做 T；砸盘里 taker 过不了门就挂 25¢ 等**。

| Play | 何时进 | 何时出 |
| --- | --- | --- |
| `lock_hold` | σ≥3.2、p≥99.8%、ask≤$0.82、费后 b 和 EV 都 ≥20% | 拿到结算；盘口若已锁 20% 也可以提前走。**不做 T、不在 TWAP 前 flatten** |
| `impulse_t` | 3 分钟 BRTI 至少动 **$100**，只做动量方向，ask $0.18–$0.52，p≥52% | **10%–50%** 兑现带；**不翻仓**；亏了下一小时空 wait、同向 taker 仍可做。**连续亏的小时不叠坐下一小时**（纸盘 `AUG2605`/`AUG2606` 同向 taker 连亏把 coupon 往后推） |
| `impulse_wait` | **dump coupon**。动量 ≤−$100，taker 过不了门，NO ask 还在 **$0.32–$0.42**、离现货 ≤$150。**挂 $0.25 maker**，不成交不算入账。已经砸到 29¢ 的不挂（纸盘 `T78499` 那刀）。淡了不撤；反弹里不填；**淡了 ask==rest 也不填**（纸盘 `AUG2604` 07:41）。纸盘 `AUG2608` 第一次打出人手那种 clip：`T78299` rest 0.25 under 0.34，砸盘里成交，`t_clip` +18% | 10%–50% 带；8 分钟没到 +10% 就 scratch；已经跑出 +10% 才用 −80% 扛反弹。涨势不挂 YES |
...
灵活的意思是：**能锁 20% 就锁；ATM 动量够格就 taker 做一笔 T；砸盘里模型 p 不够、卖一还在 25¢ 之上就挂 maker 等；锁过这小时不再开 T；10%–50% 都算正常兑现；不翻面。** 不是每笔保证 20%。没有票就空仓。不要把便宜 NO 改成 taker——那条带子已经对照过，更亏。
