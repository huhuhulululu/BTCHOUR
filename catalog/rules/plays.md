...
| `impulse_t` | 3 分钟 BRTI 至少动 **$100**，只做动量方向，ask $0.18–$0.52，p≥52% | **10%–50%** 兑现带；**不翻仓**；亏了下一小时空 wait、同向 taker 仍可做。**已经在 skip 小时里再亏，不再连空下一小时**（纸盘 `AUG2605`/`AUG2606` 同向 taker 连亏把 coupon 往后推） |
| `impulse_wait` | **dump coupon**。动量 ≤−$100，taker 过不了门，NO ask 还在 **$0.32–$0.42**、离现货 ≤$150。**挂 $0.25 maker**，不成交不算入账。已经砸到 29¢ 的不挂（纸盘 `T78499` 那刀）。淡了不撤；反弹里不填；**淡了 ask==rest 也不填**（纸盘 `AUG2604` 07:41） | 10%–50% 带；8 分钟没到 +10% 就 scratch；已经跑出 +10% 才用 −80% 扛反弹。涨势不挂 YES |
