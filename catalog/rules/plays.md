...
| `impulse_t` | 3 分钟 BRTI 至少动 **$100**，只做动量方向，ask $0.18–$0.52 | 10% 碰到就走；**不翻仓**；亏了下一小时空仓 |
| `swing_t` | 小时/15m、距行权价 ≤$600、ask $0.18–$0.72、p≥55%、p−ask≥8% | 20% 锁定 / **10% clip 立刻走** / 4¢ 回撤 / p 淡化 12 点；同一小时一张合约，clip 后也不翻面 |
...
- 做T：动量方向、ask $0.18–$0.52、目标 **10% 落袋**、不翻面（见 [`swing.md`](swing.md)）
- 失效：`INVALIDATE_P=0.40`（只作用于非锁仓仓位）
- 收盘前：`FLATTEN_SECONDS=40`
- `ALLOW_EARLY_EXIT=1`

灵活的意思是：**能锁 20% 就锁；有明确动量就做一笔 T；10% 就走；模型塌了就砍；不翻面。** 不是每笔保证 20%。做T 的 10% 是出货下限。没有票就空仓。
