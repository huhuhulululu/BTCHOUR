...
**`impulse_wait`** 是人手砸盘单：卖一还在 25–48¢、模型 p 过不了 52% 时，**不要吃卖一**，在 **$0.25** 挂 maker。回放按 rest 价成交（maker 费 0），不是按打穿后的卖一。成交后扛过反弹（不用 −12% 止损 / 不用 p 淡化），10%–50% 带兑现；亏 50% 才砍。卖一已经 ≤$0.25 就空着——那是便宜 taker 对照里亏的路径。`BTCHOUR_IMPULSE_WAIT=0` 可关。
