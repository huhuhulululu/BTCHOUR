...
> 不 `t_wait_stop`、不 `t_scratch`、不 TWAP flatten。
>
> **⚠ 2026-09-04：当初支撑这条的数字已撤回（ADR 039）。默认值不动。**
> 「持有比出场栈好 3.6¢」是 `study_coupon.py` 的记账 bug——50% 封顶按 `cap_price` 记，
> 生产卖在 **bid**（本仓库 `exits.py:145`）。封顶是最常见的出场、又是**赢家**那一支。
> 改那一行：出场栈 **−3.75¢ → +0.282¢**。
> 而且报的 t 检验的是「clip≠0」不是「clip 比 hold 差」；**配对差 +3.226¢、t=0.93、
> CI 含零**，hold 只在 **28%** 的单笔上赢、中位 **−4.667¢**。
> **高方差的一侧是持有，不是 clip。**
> 两条手臂都是统计上的零，所以**没有证据说持有有害**——默认保留是合理的，
> 但不要再引用「省 3.6¢」。`BTCHOUR_IMPULSE_WAIT_HOLD=0` 恢复旧栈作为对照。
>
