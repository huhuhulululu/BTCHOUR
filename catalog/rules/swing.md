...
`flex` 扫描顺序：**`lock_hold` → `impulse_t` → `swing_t` → `lock_wait`**。已经决定、还能吃到 $0.82 的票，先锁，不做 T。

**`impulse_t`** 跟着 3 分钟 BRTI 动量走，不要求 `p − ask` 缺口。涨了至少 **$80** 只做 YES，跌了只做 NO。做完这一笔（无论 clip 还是 fade）这小时不再开新 T，不翻对面报仇。
