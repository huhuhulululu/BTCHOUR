...
**`impulse_wait`** 是人手砸盘单：**只挂 NO**。卖一还在 25–48¢、模型 p 过不了 52% 时，**不要吃卖一**，在 **$0.25** 挂 maker。挂哪张：选**离现货最近的行权价**，不要选卖一刚好在 25¢ 上面最便宜的那张（`AUG2602` `T78499` ask 0.29 赢了更近的 `T78599` ask 0.42，反弹把那张 NO 标到 3¢，`t_wait_stop` −89%）。3 分钟印从 −$112 淡到 −$87 **不撤**；只有对面出现 ≥+$100 的涨势才撤。回放按 rest 价成交（maker 费 0），分钟高点打穿 rest 也算；**反弹里不填**（`AUG2520` 23:14 YES 拉到 0.86 时填 25¢，下一分钟就被标到 13¢）。卖一打到 rest 算成交。成交后扛过反弹（不用 −12% 止损 / 不用 p 淡化 / 不用 40% 失效），10%–50% 带兑现；亏 **80%** 才砍——人手那笔在反弹里浮亏过 40%+。涨势不对称挂 YES。卖一已经 ≤$0.25 就空着。`BTCHOUR_IMPULSE_WAIT=0` 可关。
