...
> 不 `t_wait_stop`、不 `t_scratch`、不 TWAP flatten。依据：1544 小时 / 66.2 天里同一批 348 张
> coupon 成交，持到结算 −0.86¢（t=−0.33），套上本页出场栈 −4.14¢（t=−4.99），日历两半都显著。
> 本页 `t_clip` / `t_trail` / `t_stop` / `t_fade` 对 **`swing_t` / `impulse_t` 仍然生效**
> （两者默认都关）。`BTCHOUR_IMPULSE_WAIT_HOLD=0` 恢复旧栈，作为 sweep 的 `flex_clipstack` 对照。
