...
小时盘和 15 分钟盘的盘口会来回晃。**做T** 不是拿到结算，也不是假装每一笔都锁 20%。做法是：顺着已经走出来的方向进，盘口给出 **10%–30%** 就走，模型塌了就砍。同一窗口**不翻另一边**——人累了选错方向，都是从翻面开始的。
...
`flex` 扫描顺序：**`lock_hold` → `impulse_t` → `lock_wait`**。已经决定、还能吃到 $0.82 的票，先锁，不做 T。`--playbook swing` 才加 `swing_t`。

**`impulse_t`** 跟着 3 分钟 BRTI 动量走。涨了至少 **$100** 只做 YES，跌了只做 NO。ask **$0.18–$0.52**。做完这一笔（赚或亏）这小时不再开新 T，**不翻对面**。亏了，**下一小时也空仓**。

同一小时只盯**第一次进场的那张合约**。不要在砸盘里换行权价，也不要人累了猜反弹。

## 何时出

1. **`lock_on_book`**：对方买价已经锁住 **20%**，直接兑现。
2. **`t_clip`**：往返 ROI ≥ **10%** 就走。不吃整段行情，不拿缺口当 runner。
3. **`t_trail`**：曾经摸到过 10%，漏了那一跳，再从最高买价回撤 **4¢** 就走。
4. **`t_stop`**：往返亏 **12%**，砍。
5. **`t_fade`**：模型 p 比进场掉了 **12 个百分点**，方向不对，先出来。
...
| `BTCHOUR_SWING_TARGET` | 0.10 |
| `BTCHOUR_SWING_TRAIL` | 0.04 |
| `BTCHOUR_SWING_FADE` | 0.12 |
| `BTCHOUR_SWING_MAX_DISTANCE` | 600 |
| `BTCHOUR_SKIP_AFTER_LOSS` | 1 |

这不是「每笔 T 都赚 10–30%」。10% 是**出货下限**：碰到就落袋。人的好单是挂进、几分钟走、同一方向。人累了会翻面、换行权价、追 0.60+ 的 YES——机器不做这三件事。空仓仍然是正确动作。
