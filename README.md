...
默认策略是 **`flex`**：已经决定的合约先按稳健 20% 锁仓；小时盘上 3 分钟 BRTI 动量再做短线 T（**10% 落袋** / −12% 硬止损 / 不翻面 / 亏了下一小时空仓）。计分：`EV = p · b − (1 − p)`。
...
默认 `BTCHOUR_MODE=paper`，`BTCHOUR_PLAYBOOK=flex`。$0.83 的锁仓等待单不成交不算入账。做T 的 10% 是出货下限，不是保证。实盘密钥只放本机 `.env`，不入库。
